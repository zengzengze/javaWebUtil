...
public class fileUtils_ms {

    private static final String path = "H:/zfj/ms/";
    private static String xlsxPath = "C:\\Users\\28065\\Desktop\\南商\\用户初始化数据-uat.xlsx";
    ...
    private static String[] head = {"usr_bran", "usr_nm", "inst_pnp_id", "sex", "chanl_no", "usr_lev"};

    private static String[][] usr_brans = {
            {"总行", "包头分行", "巴彦淖尔分行", "阿拉善分行", "兴安盟分行", "赤峰分行", "鄂尔多斯分行",
                    "二连浩特分行", "呼伦贝尔分行", "满洲里分行", "通辽分行", "乌兰察布分行", "总行营业部", "乌海分行", "呼和浩特分行", "锡林郭勒分行"},
            {"111111111", "200010000", "800030000", "901160000", "201120000", "400020000", "600060000",
        ...
        for (int row = 1; row < (maxRow + 1); row++) {
            ...
            for (int col = 0; col < maxRol; col++) {
                ...
                if (col == 4) {
                    ...
                    fileUtils_ms chanlDetail = (fileUtils_ms) chanlMap.get(usrLev);
    ...
    private static void chanlInfoMapInit() {
        ...
        for (int i = 1; i < maxRow3 + 1; i++) {
            ...
            HashMap<String, fileUtils_ms> chanlMap = new HashMap<>();
            for (int row = 1; row < maxRow2 + 1; row++) {
                fileUtils_ms chanlDetail = new fileUtils_ms();
